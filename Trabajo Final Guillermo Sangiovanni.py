...
#Division de datos y validacion cruzada
...
Se utilizará el método KNNImputer para completar valores faltantes. Este enfoque considera la similitud entre filas (vecinos más cercanos) y es más preciso que simplemente usar la media o mediana cuando las variables tienen correlaciones entre sí.

Por ejemplo, si un celular tiene valores similares en RAM y Storage a otros, el imputador tomará eso en cuenta para estimar el valor faltante.
"""
...
### ✅ Modelos seleccionados y estrategia de validación

Para predecir el precio final de celulares, se trabajó con un enfoque progresivo que permitió evaluar el impacto de la complejidad del modelo y la técnica de validación. Se aplicaron cuatro combinaciones distintas:

---

#### 🔹 1. **Regresión Lineal con Validación Hold-Out**
Se utilizó como modelo base por su simplicidad, velocidad de entrenamiento y facilidad de interpretación. Permite observar la relación directa entre variables independientes (como RAM, almacenamiento, marca) y el precio final. En este caso, se dividió el conjunto de datos en entrenamiento y test, utilizando la técnica de Hold-Out (porcentaje fijo, por ejemplo 80/20).

---

#### 🔹 2. **Random Forest Regressor con Validación Hold-Out**
Luego se aplicó un modelo más robusto, Random Forest, también con Hold-Out. Random Forest es un algoritmo de ensamblado que crea múltiples árboles de decisión y promedia sus predicciones, lo que lo hace ideal para captar relaciones no lineales y reducir el sobreajuste. Esta primera implementación permitió comparar contra el modelo lineal manteniendo la misma forma de validación.

---

#### 🔹 3. **Regresión Lineal con Validación Cruzada (K-Fold)**
En esta etapa se volvió a utilizar regresión lineal, pero evaluada mediante validación cruzada con **K-Fold (k=5)**. Esta técnica divide el dataset en 5 partes: entrena con 4 y valida con 1, repitiendo el proceso hasta rotar todos los subconjuntos. Así se obtienen métricas más estables y confiables, al minimizar el sesgo de una única partición.

---

#### 🔹 4. **Random Forest Regressor con Validación Cruzada (K-Fold)**
Finalmente, se utilizó Random Forest nuevamente, pero esta vez con K-Fold. Esto permitió evaluar su desempeño de forma más sólida, y confirmar si las mejoras observadas con Hold-Out se mantenían con una validación más estricta.

---

### ✅ Conclusión metodológica

Esta estrategia comparativa (modelos simples vs complejos, y validación Hold-Out vs K-Fold) permitió identificar no solo qué modelo predice mejor, sino también **cuán confiables son esas predicciones** en distintos escenarios. Esto refuerza la elección final del modelo más adecuado según el comportamiento real del dataset.
...
# Scatter de puntos predichos vs reales
sns.scatterplot(x=y_test, y=y_pred, alpha=0.6, label="Predicciones")

# Línea ideal donde y_pred = y_test (predicción perfecta)
min_val = min(y_test.min(), y_pred.min())
max_val = max(y_test.max(), y_pred.max())
plt.plot([min_val, max_val], [min_val, max_val], 'r--', label="Línea perfecta de prediccion")
...
plt.figure(figsize=(6, 6))
sns.scatterplot(x=y_test, y=y_pred_rf, alpha=0.6, label="Predicciones")
